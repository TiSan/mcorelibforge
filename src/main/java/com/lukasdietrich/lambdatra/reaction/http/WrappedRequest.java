...
/**
 * Wraps {@link HttpRequest} into a simpler class and combines it with access to
 * parsed url query and parameters.
 * 
 * @author Lukas Dietrich, TiSan
 *
 */
public final class WrappedRequest<S> {
	...
	public HttpPostRequestDecoder parseBody() {
		return new HttpPostRequestDecoder(new DefaultHttpDataFactory(
				Short.MAX_VALUE), req);
	}
	...
	public Optional<S> getSession() {
		...
		return (cookie.isPresent()) ? sessions.getSession(cookie.get().value())
				: Optional.empty();
	}
	...
	/**
	 * Returns the request path omitting the query part. <br>
	...
	/**
	 * Returns the Queries in a List, not a map.
	 * @return the queries in a list.
	 */
	public List<Query> getQueries() {
		ArrayList<Query> queries = new ArrayList<Query>();
		if (query.parameters() != null && !query.parameters().isEmpty()) {
			for (String s : query.parameters().keySet()) {
				queries.add(new Query(s, query.parameters().get(s)));
			}
		}
	...
	public class Query {
		...
		private Query(String queryName, List<String> value) {
